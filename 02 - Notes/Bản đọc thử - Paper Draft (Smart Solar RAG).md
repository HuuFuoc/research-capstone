...
**Xếp hạng hiệu quả & truy hồi tiếng Việt.** Ở tầng xếp hạng, tiêu chí của chúng tôi là cơ chế re-rank đủ nhẹ cho hạ tầng dự án. ColBERT (Khattab and Zaharia, 2020) đáp ứng nhờ thiết kế *late interaction*: biểu diễn tài liệu được tính sẵn offline bằng BERT, lúc truy vấn chỉ còn bước so khớp mịn ở mức token (MaxSim). Theo số liệu nhóm tác giả báo cáo, cách tách hai pha này cắt chi phí mỗi truy vấn khoảng **bốn bậc độ lớn FLOPs** và rút ngắn thời gian xử lý khoảng **hai bậc độ lớn**, trong khi độ chính xác vẫn thuộc nhóm các mô hình nền BERT. Với corpus **tiếng Việt**, Nguyen Ba et al. (2024) xử lý các thách thức đặc thù (token quá dài, ensemble thiếu ổn định, thứ tự tài liệu tham chiếu bị bỏ qua) bằng tiền xử lý dữ liệu, **Reciprocal Rank Fusion chuẩn hóa thứ tự** (ghép keyword + vector) và **re-rank bằng Active Retrieval** — tham chiếu kỹ thuật sát nếu tài liệu O&M của dự án bằng tiếng Việt.

**Đánh giá truy hồi trên tài liệu kỹ thuật & câu hỏi đa đoạn.** Hai benchmark gần đây làm rõ độ khó đúng bối cảnh đề tài. FreshStack (Thakur et al., 2025) dựng benchmark truy hồi *thực tế, không nhiễm dữ liệu* trên tài liệu kỹ thuật/mã nguồn; ở **cả năm chủ đề khảo sát, không retriever nào dùng out-of-the-box tiến gần được mức oracle** — dư địa cải thiện chất lượng truy hồi còn rất lớn. Với lớp **câu hỏi đa đoạn** — loại câu hỏi buộc hệ thống gom bằng chứng từ nhiều đoạn rời rạc rồi suy luận trên đó — kết quả benchmark của MultiHop-RAG (Tang and Yang, 2024) cho thấy chất lượng trả lời của các hệ RAG hiện có vẫn ở mức thấp. Hai kết quả này thúc đẩy việc phân loại câu hỏi *factoid / quy trình / đa-đoạn* trong test set của chúng tôi (Mục 5.1) và đặt kỳ vọng thực tế cho retriever trên tài liệu O&M dài, chuyên ngành.
